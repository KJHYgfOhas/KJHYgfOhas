...
# https://github.com/jkjung-avt/keras_imagenet/blob/master/utils/image_processing.py

"""
Provides utilities to preprocess images.

Data agumentation does the following:
...
def resize_and_rescale_image(image, height, width,
                             do_mean_subtraction=True, scope='resize_image'):
    ...
    """
    with tf.name_scope(name=scope):
        image = _aspect_preserving_resize(image, max(height, width)+15)
        image = _central_crop([image], height, width)[0]
        if do_mean_subtraction:
            # rescale to [-1,1]
            # image = tf.Print(image, [tf.math.reduce_max(image), tf.math.reduce_min(image)], message="\n\nMAX and MIN VALUE:")
...
def preprocess_for_train(image,
  ...
  with tf.name_scope(name=scope):
    ...
    resized_image = _aspect_preserving_resize(rotated_image, max(height, width)+15)
    cropped_image = tf.image.random_crop(resized_image, [height, width, 3])
    # cropped_image.set_shape([height, width, 3])

    # Randomly flip the image horizontally.
    processed_image = tf.image.random_flip_left_right(cropped_image)

    # Randomly distort the colors. There are 1 or 4 ways to do it.
    num_distort_cases = 1 if fast_mode else 4
    processed_image = apply_with_random_selector(
        processed_image,
